...
class MetricLogger(object):
    ...
    def global_avg(self):
        ...
        if self.log_fh:
            log_message = f"Averaged stats: {self.delimiter.join(loss_str)}\n"
            self.log_fh.write(log_message)
            self.log_fh.flush()
        return self.delimiter.join(loss_str)
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        final_output = '{} Total time: {} ({:.4f} s / it)'.format(
            header, total_time_str, total_time / len(iterable))
        print(final_output)
        if self.log_fh:
            self.log_fh.write(final_output + '\n')
            self.log_fh.flush()
