...
LastEditTime: 2025-02-21 15:06:33
'''
import os
import json
import torch
...
from models.scracap import SCRACap
...
def validation_whoops(
    ...
    predicts = []
    examples = load_dataset('nlphuji/whoops', token='')
    model.eval()
    ...
    out_json_path = os.path.join(args.out_path, f'{args.name_of_datasets}_generated_captions_vote.json')
...
def validation_coco_flickr30k(
    ...
    for idx, item in tqdm(enumerate(annotations['annotations'])):
        image_id = annotations['images'][idx]['file_name']
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            bos_embeds = model.llama_model.model.embed_tokens(bos)
            embeddings = torch.cat([bos_embeds, prompt_embeds], dim=1)
            sentence = beam_search(embeddings = embeddings, tokenizer = tokenizer, beam_width = args.beam_width, model = model.llama_model, max_len=300) # List[str]
            sentence = sentence[0]
            print('Pred: ', sentence)
    ...
    if not os.path.exists(args.out_path):
        os.makedirs(args.out_path, exist_ok=True)
    out_json_path = os.path.join(args.out_path, f'{args.name_of_datasets}_best.json')
    with open(out_json_path, 'w') as outfile:
        json.dump(predicts, outfile, indent = 4)
...
def main(args) -> None:
    ...
    print('load:', ckpt)
    model = SCRACap(
            ext_path = 'ext_data/ext_memory_miccai_new.pkl',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_of_val_datasets', default = './data/miccai_coco/annotations/val.json')
    parser.add_argument('--image_folder', default = './data/miccai_coco/val/')
