...
class SCRACap(Blip2Base):
    ...
    def __init__(
        ...
        topn=9,
        llama_model="",
        prompt_path="prompts/prompt_scracap.txt",
        ...
        with open(ext_path, 'rb') as f:
            ext_base_img, self.ext_base_img_id, self.ext_base_triplet = pickle.load(f)
    ...
    def label_to_name(self, labels, knowledge_type='tissue'):
        categories = [
            'Bipolar Forceps', 'Large Needle Driver', 'Prograsp Forceps',
            'Monopolar Curved Scissors', 'Clip Applier', 'Suction', 'Stapler',
            'Ultrasound Probe', 'Loop', 'Manipulate', 'Null_Verb', 'Clip',
            'Cauterize', 'Retract', 'Grasp', 'Cut', 'Suction', 'Suture',
            'Staple',
            'Sense', 'Tissue', 'Null_Target', 'Blood', 'Vessels'
        ]
        if knowledge_type == 'instrument':
            categories = categories[:8]
            labels = labels[:8]
        elif knowledge_type == 'action':
            categories = categories[8:20]
            labels = labels[8:20]
        elif knowledge_type == 'tissue':
            categories = categories[20:]
            labels = labels[20:]
        else:
            raise ValueError(f"Unknown knowledge_type: '{knowledge_type}'. Expected one of ['instrument', 'action', 'tissue']")

        # Generate a list of categories based on where the one-hot encoding is 1
        category_names = [categories[i].lower() for i, label in enumerate(labels) if
                          label == 1]
    ...
    def retrieve_similar_features(self, query_features, feat_index, image_id, top_k = 5, sub_top_k = 32):
        ...
        for listA, listB in zip(top_k_similarities, re_txt_list_all):
            ...
            sorted_listB = [self.label_to_name(listB[idx]) for idx in indices]
            sorted_listB = sorted_listB[:sub_top_k]
            sorted_listB = [item for sublist in sorted_listB for item in
                            sublist]
    ...
    def encode_img(self, image):
        ...
        with self.maybe_autocast():
            ...
            # 统计每个字符串在 re_txt_list_all 中的出现次数
            all_items = [item for sublist in re_txt_list_all for item in
                         sublist]
            ...
            for sublist in re_txt_list_all:
                sublist_new = []
                for item in sublist:
                    if item_counts[item] > 10 and item not in sublist_new:
                        sublist_new.append(item)
                        if len(sublist_new) > self.topn:
                            break
                sublist_new = list(filter(lambda x: x not in ['null_target', 'null_verb'], sublist_new))
...
class SCRACAP_IMG(Blip2Base):
    ...
    def img_vector(self, image):
        device = image.device

        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image)).to(
                device)
            ...
            query_output_img = query_outputs_img.last_hidden_state
            img_embedding_mean = torch.mean(query_output_img, dim=1)
        return img_embedding_mean